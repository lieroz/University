...
void Drawer::drawPoints(QGraphicsScene* scene, const QVector<QPoint>& first_set, const QVector<QPoint>& second_set) {
    Drawer::drawSetPoints(scene, first_set, Qt::red);
    Drawer::drawSetPoints(scene, second_set, Qt::blue);
}

void Drawer::drawLine(QGraphicsScene* scene, const QPoint& A, const QPoint& B, const QColor& color) {
    QPen pen = QPen{color};
    pen.setWidth(3);
    scene->addLine(A.x(), A.y(), B.x(), B.y(), pen);
}

void Drawer::drawHeights(QGraphicsScene* scene, const QVector<QPoint>& points, const QColor& color) {
    for (int i{}; i < points.size() - 1; ++i) {
        scene->addLine(points.at(3).x(), points.at(3).y(), points.at(i).x(), points.at(i).y(), QPen{color});
    }
}

void Drawer::drawText(QGraphicsScene* scene, const QVector<QPoint>& points) {
    for (int i{}; i < points.size() - 1; ++i) {
...
void Drawer::drawTriangle(QGraphicsScene* scene, const QVector<QPoint>& points, const QColor& color) {
...
void Drawer::drawAnswer(QGraphicsScene* scene, const QVector<QPoint>& first, const QVector<QPoint>& second) {
